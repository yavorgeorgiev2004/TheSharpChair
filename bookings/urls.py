...
urlpatterns = [
    ...
    # e.g. /book/confirmed/SC3F7A9B2E/ → booking_confirmed(request, ref='SC3F7A9B2E')
    ...
    path('schedule/',
         ...
         name='barber_schedule'),  # only barbers can see this — role checked in view


    # ── AJAX — internal JSON API ──────────────────────────────────────────────
    # Called by JavaScript fetch() in book_step3.html when the user picks a date.
