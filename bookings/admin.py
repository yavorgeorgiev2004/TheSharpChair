...
Each ModelAdmin class customises how that model is displayed:
  ...
  list_editable  → fields editable inline in the list (no need to open each record)
...
2. /admin → Barbers → Add Barber → select that user, add speciality, tick is_available
...
class ServiceAdmin(admin.ModelAdmin):
    ...
    COMMON TASKS:
    - Add new service: Add Service button, fill in name/description/price/duration
    ...
    # list_editable allows editing price and is_active directly in the list view
...
class BookingAdmin(admin.ModelAdmin):
    ...
    list_display   = ['ref', 'customer', 'barber', 'service', 'date', 'start_time', 'status']
    ...
    search_fields  = ['ref', 'customer__first_name', 'customer__last_name', 'customer__email']
